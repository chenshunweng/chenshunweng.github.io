...
import { X, Briefcase, BookOpen, ExternalLink, Video, Image as ImageIcon } from 'lucide-react';
...
const Projects = () => {
  ...
  useEffect(() => {
    const loadProjects = () => {
      const loadedProjects = projectKeys.map(key => ({
        ...
        image: `/assets/images/${key}.jpg`, // Placeholder image path
        video: `/assets/videos/${key}.mp4`  // Placeholder video path
      }));
  ...
  const getProjectNameForFile = (projectKey) => {
    // Simplified: use the key directly or map to a more filename-friendly string if needed
    return projectKey.replace(/project_|_title/g, '');
  };
  ...
  const FilterButton = ({ type, label }) => (
    <button
      ...
      className={`px-4 py-2 rounded-lg transition-all duration-300 ease-in-out font-medium 
                  ${filter === type 
                    ? (theme === 'dark' ? 'bg-primary text-gray-900' : 'bg-primary text-white') 
                    : (theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600 text-gray-300' : 'bg-gray-200 hover:bg-gray-300 text-gray-700')}
                  focus:outline-none focus:ring-2 ${theme === 'dark' ? 'focus:ring-primary-light' : 'focus:ring-primary'}`}
    >
              ...
              <div className="p-6">
                ...
                <button 
                  className={`w-full py-2 px-4 rounded-lg font-semibold transition-colors duration-300 
                              ${theme === 'dark' ? 'bg-primary text-gray-900 hover:bg-primary-dark' : 'bg-primary text-white hover:bg-primary-dark'}`}
                >
            ...
            >
              ...
              <div className="prose prose-sm md:prose-base max-w-none dark:prose-invert">
                ...
                {selectedProject.result && (
                  <div className="mt-4">
                    <h4 className="font-semibold text-lg mb-1">{t('project_modal_result_label')}</h4>
                    <p className="whitespace-pre-line">{selectedProject.result}</p>
                  </div>
                )}
              </div>
